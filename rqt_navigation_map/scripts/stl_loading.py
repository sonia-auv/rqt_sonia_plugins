...
class loader:
    ...
    def draw(self):
        glBegin(GL_TRIANGLES)  # pylint:disable=undefined-variable
...
class draw_scene:
    def __init__(self, style=1):
        ...
        self.model1.load_stl('base_link.STL')
        self.init_shading()

    # solid model with a light / shading
    def init_shading(self):
        glShadeModel(GL_SMOOTH)  # pylint:disable=undefined-variable
        glClearColor(0.0, 0.0, 0.0, 0.0)
        glClearDepth(1.0)
        glEnable(GL_DEPTH_TEST)  # pylint:disable=undefined-variable
        glShadeModel(GL_SMOOTH)  # pylint:disable=undefined-variable
        glDepthFunc(GL_LEQUAL)  # pylint:disable=undefined-variable
        glHint(GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST)  # pylint:disable=undefined-variable

        glEnable(GL_COLOR_MATERIAL)  # pylint:disable=undefined-variable
        glEnable(GL_LIGHTING)  # pylint:disable=undefined-variable
        glEnable(GL_LIGHT0)  # pylint:disable=undefined-variable
        glLight(GL_LIGHT0, GL_POSITION, (0, 1, 1, 0))  # pylint:disable=undefined-variable
        glMatrixMode(GL_MODELVIEW)  # pylint:disable=undefined-variable

    def resize(self, (width, height)):
        if height == 0:
            height = 1
        glViewport(0, 0, width, height)
        glMatrixMode(GL_PROJECTION)  # pylint:disable=undefined-variable
        glLoadIdentity()
    ...
    def draw(self):
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)  # pylint:disable=undefined-variable
        glLoadIdentity()
...
def main():
    # initalize pygame
    pygame.init()
    pygame.display.set_mode((640, 480), OPENGL | DOUBLEBUF)  # pylint:disable=undefined-variable
